...
assert.match(push,/async function disable\s*\(/,'push client must expose a disable path');
assert.ok(push.includes('await removeFromCloud(endpoint)'),'disable must remove server reachability');
assert.ok(push.includes('await sub.unsubscribe()'),'disable must unsubscribe the browser endpoint');
assert.ok(push.includes('window.GamenfyPush = { enable, disable, status, isStandalone }'),'public push API must include disable');
assert.ok(push.indexOf('await removeFromCloud(endpoint)') < push.indexOf('await sub.unsubscribe()'),'cloud opt-out must happen before local unsubscribe so failures cannot be reported as success');
