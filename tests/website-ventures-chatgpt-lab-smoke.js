...
assert.ok(lab.includes('30×')&&lab.includes('10×')&&lab.includes('3×'),'Historical evidence gates remain visible in the business experiment surface');
...
assert.ok(lab.includes('ventures-sales.html'),'Business Lab keeps historical Sales Machine access');
...
assert.ok(sites.includes('ventures-sales-v2.html'),'Website Lab must surface the active Sales Machine v2');
assert.ok(sites.includes('website-ventures-visual-vault.html'),'Website Lab must surface the Visual Vault');
assert.ok(sites.includes('website-ventures-production-line-v3.html'),'Website Lab must surface the current production path');
...
assert.equal(labState.activePrototype.currentLabVersion,'v1.5','Lab state must track current Agency Showroom version');
