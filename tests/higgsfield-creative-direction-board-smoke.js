...
assert.ok(line.includes('Higgsfield Batch 4 complete · Batch 5 next'),'Production Line v3 surfaces current batch state');
assert.ok(workspace.includes('Higgsfield Queue →'),'Ventures overview links the live generation queue');
assert.ok(workspace.includes('ventures.js?v=11.2'),'Ventures workspace busts cache for the refreshed task queue');
assert.ok(ventures.includes('const GAMENFY_FOCUS_VERSION = 3'),'Gamenfy Builder task queue is on the current focus version');
assert.ok(ventures.includes("title: 'Batch 5 · Agency Hero Round'"),'Gamenfy Builder next step starts at Batch 5');
