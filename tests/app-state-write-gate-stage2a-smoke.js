...
assert.doesNotMatch(sql, /gamenfy_restore_app_state/i,
  'Stage 2A must not expose any restore RPC');

const fn = (sql.match(/create or replace function public\.gamenfy_write_app_state\([\s\S]*?\$\$;/i) || [])[0] || '';
...
assert.doesNotMatch(fn, /on conflict/i,
...
assert.doesNotMatch(sql, /revoke\s+(?:insert|update|delete)[\s\S]*?on table public\.app_state/i,
  'Stage 2A is additive and must not lock old clients out yet');
assert.doesNotMatch(sql, /service_role/i,
  'Stage 2A must not alter service-role behavior');
assert.doesNotMatch(sql, /drop constraint|drop primary key|drop column/i,
