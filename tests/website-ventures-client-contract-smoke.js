...
assert.ok(bootstrap.indexOf("loadScriptOnce('website-ventures-client-contract.js')") < bootstrap.indexOf("loadScriptOnce('website-ventures-client-config.js')"), 'shared contract must load before the client loader');
assert.ok(factory.includes('<script src="website-ventures-client-contract.js"></script>'), 'Client Factory must load the shared contract');
