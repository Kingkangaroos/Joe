...
assert.ok(src.includes(".select('key,data,updated_at,restore_generation')"), 'backup must read cloud timestamps and restore generation for dirty ordering');
...
assert.ok(src.includes('const remoteGeneration = Math.max(0, Math.floor(Number(row.restore_generation) || 0))'),
  'backup must normalize the exact cloud restore generation before considering local dirty edits');
assert.ok(src.includes('const itemGeneration = Math.max(0, Math.floor(Number(item.generation) || 0))'),
  'backup must normalize dirty-entry generation; legacy entries intentionally map to generation zero');
assert.ok(src.includes('itemGeneration !== remoteGeneration'),
  'dirty edits from another restore generation must never overlay the cloud snapshot');
...
assert.ok(src.includes('pendingLocalApplied: pendingApplied'),
  'backup must record whether newer same-generation local pending edits were included');
...
console.log('Cloud backup smoke: owner cloud is authoritative, only newer same-generation dirty edits are preserved, sensitive keys are stripped, and stale/old-generation device cache cannot masquerade as a complete backup.');
